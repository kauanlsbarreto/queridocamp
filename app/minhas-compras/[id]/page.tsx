...
export default function MinhasComprasPage() {
  ...
  useEffect(() => {
    ...
    const loadPayments = async () => {
      ...
      try {
        const res = await fetch(`/api/loja/pagamento/minhas?faceit_guid=${encodeURIComponent(faceitGuid)}`, {
          headers: { "x-faceit-guid": faceitGuid },
          cache: "no-store",
        });
        ...
        if (!res.ok) {
          setPayments([]);
          setError(data?.message || "Nao foi possivel carregar suas compras.");
          return;
        }

        const all = Array.isArray(data?.payments) ? (data.payments as PaymentItem[]) : [];
        setPayments(all.filter((payment) => String(payment.status || "").toUpperCase() === "PAID"));
      } catch {
        setPayments([]);
        setError("Erro inesperado ao carregar suas compras.");
      } finally {
        setLoading(false);
      }
    };
        ...
        <div className="container mx-auto max-w-5xl px-4 space-y-6">
          <PremiumCard>
            <div className="p-6 md:p-8">
              ...
              <h1 className="mt-1 text-3xl font-black uppercase text-white">Minhas Compras</h1>
              <p className="mt-2 text-sm text-zinc-400">Historico de pagamentos aprovados do seu usuario.</p>
              <div className="mt-4">
                <Link
                  href="/loja"
          ...
          {allowed && loading && (
            <PremiumCard>
              <div className="p-6 text-sm text-zinc-300">Carregando compras...</div>
            </PremiumCard>
          )}

          {allowed && !loading && !payments.length && !error && (
            <PremiumCard>
              <div className="p-6 text-sm text-zinc-300">Nenhuma compra aprovada encontrada para seu usuario.</div>
            </PremiumCard>
          )}
              ...
              {payments.map((payment) => (
                <PremiumCard key={payment.id}>
                  <div className="space-y-2 p-5">
                    <p className="text-xs uppercase tracking-[0.12em] text-zinc-500">Compra #{payment.id}</p>
                    ...
                    <p className="text-sm text-zinc-300">Metodo: {payment.method}</p>
                    ...
                    <p className="text-xs text-zinc-500">Pago em: {formatDateTime(payment.paidAt)}</p>
                  </div>
                </PremiumCard>
              ))}
            </div>
