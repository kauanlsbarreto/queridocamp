...
export async function POST(req: Request) {
  try {
    ...
    // log this successful login into logs_logins table and also send the
    // webhook so that every authentication attempt (even those which ignore
    // the client) will be reported.
    try {
      await connection.query(`
      ...
      console.error("failed to write login log in /api/players", e);
    }
